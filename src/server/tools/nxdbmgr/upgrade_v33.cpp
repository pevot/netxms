...
static bool H_UpgradeFromV5()
{
   ...
   if (hResult != NULL)
   {
      ...
      {
         DBGetField(hResult, 0, 0, ports, MAX_SECRET_LENGTH);
      }
   ...
   if (ports[0] != 0)
   {
      ...
      for (int i = 0; i < portList.size(); i++)
      {
         ...
         TCHAR query[1024];
         _sntprintf(query, 1024, _T("INSERT INTO snmp_ports (id,port,zone) VALUES(%d,%d,%d)"), id++, port, -1);
   ...
   if (hResult != NULL)
   {
      ...
      for (int j = 0; j < count; j++)
      {
         ...
         DBGetField(hResult, j, 1, ports, MAX_SECRET_LENGTH);
